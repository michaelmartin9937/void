...
export function arePostsRelated(
  ...
  const postOneTags = postOne.data.tags.map((tag) => slugify(tag));
  const postOneCategories = postOne.data.categories.map((category) =>
    slugify(category),
  );

  const postTwoTags = postTwo.data.tags.map((tag) => slugify(tag));
  const postTwoCategories = postTwo.data.categories.map((category) =>
    slugify(category),
  );

  // if any tags or categories match, return true
  const tagsMatch = postOneTags.some((tag) => postTwoTags.includes(tag));
